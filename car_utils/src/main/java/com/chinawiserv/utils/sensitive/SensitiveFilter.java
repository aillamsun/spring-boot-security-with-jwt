package com.chinawiserv.utils.sensitive;
...
public class SensitiveFilter implements Serializable {
    ...
    /**
     * 默认的单例，使用自带的敏感词库
     * ClassLoader.getSystemResourceAsStream("sensitive_words.txt")
     */
    public final String DEFAULT = "sensitive_words";
     ...
     */
    public SensitiveFilter(String path) {
        sensitiveFilter(path);
    }

    public void defaultSensitiveFilter() {
        filter(DEFAULT);
    }


    public void sensitiveFilter(String path) {
        filter(path);
    }
    ...
    private void filter(String path) {
        try {
            Resource wordsPath = new ClassPathResource(path);
            BufferedReader reader = new BufferedReader(new InputStreamReader(wordsPath.getInputStream(), StandardCharsets.UTF_8));
...
//        }
     ...
     */
    public String filter(String sentence, char replace, Set<String> hitWords) {
        // 先转换为StringPointer
